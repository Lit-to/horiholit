...
execute if score temp calc < @s point run execute store result bossbar holit: value run data get storage holit: frequency[-1] 1 
#

execute if score @s point < temp calc run bossbar set holit: color green 
execute if score @s point < temp calc run execute store result bossbar holit: value run scoreboard players get @s point 
